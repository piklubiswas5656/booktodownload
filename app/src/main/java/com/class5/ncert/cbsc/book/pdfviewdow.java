...
//        url = getIntent().getStringExtra("URL");
        baseurl = "https://www.tutorialspoint.com/java/";

//        chiled = URLUtil.guessFileName(url, null, null);
        chiled = "java_tutorial.pdf";
        url = baseurl + chiled;
        fileexits();
...
//        File pdfFile = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS) + "/" + chiled);
        File pdfFile = new File(getExternalFilesDir("/").getAbsolutePath()+ "/" + chiled);
        if (pdfFile.exists()) //Checking if the file exists or not
        {
            ...
            File file1= new File(getExternalFilesDir("/").getAbsolutePath());
            PRDownloader.download(url, file1.getPath(), URLUtil.guessFileName(url, null, null))
                    .build()
...
//        String pdfFilepath = String.valueOf(new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS) + "/" + chiled));
        String pdfFilepath = String.valueOf(new File(getExternalFilesDir("/").getAbsolutePath()+ "/" + chiled));
        if (pdfFile.exists()) //Checking if the file exists or not
        {
